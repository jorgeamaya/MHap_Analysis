...
plot_coverage = function(ampseq, variable){
  ...
  plot_read_depth_heatmap = coverage %>% ggplot(aes(x = Markers, y = factor(Sample_id, levels = sample_order), fill = log10(Read_depth)))+
    geom_tile()+
  ...
  plot_read_depth_violin = coverage %>% ggplot(aes(x = var, y = log10(Read_depth), color = var))+
    geom_violin()+
  ...
  plot_read_depth_violin_by_sample = coverage %>% group_by(var, Sample_id) %>% summarise(Total_Read_depth = sum(Read_depth, na.rm = T)) %>% ggplot(aes(x = var, y = log10(Total_Read_depth), color = var))+
    geom_violin()+
    geom_jitter(alpha = .1, width = .2)+
...
get_polygenomic = function(ampseq_object, strata, update_popsummary = T, na.rm = FALSE, filters = c('Buenaventura', 'Guapi', 'Quibdo')){
...
concatenate_genes <- function(gene_names) {
  if (length(gene_names) == 0) {
    stop("Input vector is empty.")
  }
  
  concatenated_string <- paste(gene_names, collapse = '|')
  return(concatenated_string)
}

format_pop_pairs <- function(pop_pairs) {
  result_vector <- gsub("(\\w+)-(\\w+)", "\\1-\\2", pop_pairs)
  result_vector <- gsub("([A-Za-z]+)[a-z]+-([A-Za-z]+)[a-z]+", "\\1-\\2", result_vector)
